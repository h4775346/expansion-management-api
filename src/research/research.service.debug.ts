...
@Injectable()
export class ResearchServiceDebug {
  private readonly logger = new Logger(ResearchServiceDebug.name);
  ...
  async findAll(user: any, query: PaginateQuery): Promise<Paginated<ResearchDoc>> {
    this.logger.log(`findAll called with user: ${JSON.stringify(user)}`);
    ...
    if (user.role !== 'admin') {
      this.logger.log(`User is not admin, filtering by client_id: ${user.id}`);
      ...
      this.logger.log(`Project IDs: ${JSON.stringify(projectIds)}`);
      ...
      this.logger.log(`Mongo query filter: ${JSON.stringify(mongoQuery)}`);
    } else {
      this.logger.log('User is admin, no filtering applied');
    }
    ...
    this.logger.log(`Found ${data.length} research documents`);
